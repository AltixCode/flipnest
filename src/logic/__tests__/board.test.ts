...
const flipAll = (cards: Card[]): Card[] =>
  cards.map((c) => ({ ...c, matched: true }));
...
describe("the daily board", () => {
  ...
  it("deals a server-only theme (an id not in the bundled THEMES list) from its own faces, instead of silently falling back to the first bundled theme", () => {
    const remoteFaces = [
      "a",
      "b",
      "c",
    ...
    const cards = dealBoard(
      7,
      "a-theme-only-content-drip-knows-about",
      remoteFaces,
    );
    expect(new Set(cards.map((c) => c.faceId))).toEqual(new Set(remoteFaces));
  });
});
...
describe("FACE_GLYPH", () => {
  it("has an entry for every face in every theme", () => {
    const missing = THEMES.flatMap((t) => t.faces).filter(
      (f) => !FACE_GLYPH[f],
    );
